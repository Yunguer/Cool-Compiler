...
def chamaFuncao(t, listaIds, listaMetodos, listaTipos):
    ...
    if t[0] == 'idType': 
        manipulaIdType(t, newListaIds, newListaTipos, newListaMetodos)
        ...
        manipulaExprLetSeta(t, newListaIds, newListaTipos, newListaMetodos)
        chamaFuncao(t[5], newListaIds, newListaMetodos, newListaTipos)
    elif t[0] == 'exprLet': 
        manipulaExprLet(t, newListaIds, newListaTipos, newListaMetodos)
        ...
        codigoGerado += geraOp(t) + '\n'   
    elif t[0] == 'comp':
        manipulaComp(t, newListaIds)
        codigoGerado += geraComp(t) + '\n' 
    elif t[0] == 'exprNew': 
        ...
        codigoGerado += geraFeatureRetornoParametro(t) + ' {\n'
        for formal in t[4]:
        ...
        codigoGerado += geraFeatureRetorno(t) + ' '
        chamaFuncao(t[3], newListaIds, newListaMetodos, newListaTipos)
    elif t[0] == 'featureAnonima':
        codigoGerado += geraFeatureAnonima(t, listaIds) 
        manipulafeatureAnonima(t, newListaIds, newListaTipos, newListaMetodos)
        codigoGerado += '\n'
        for formal in t[2]:
            chamaFuncao(formal, newListaIds, newListaMetodos, newListaTipos)
...
def manipulaIdType(t, listaIds, listaTipos, listaMetodos):
    if len(t) == 4:
        aux = ('featureAnonima', t[1], t[2], t[3])
        manipulafeatureAnonima(aux, listaIds, listaTipos, listaMetodos)
    elif len(t) == 3:
...
def manipulaExprLetSeta(t, listaIds, listaTipos, listaMetodos):
    aux = ('featureAnonima', t[1], t[2], t[3])
    manipulafeatureAnonima(aux, listaIds, listaTipos, listaMetodos)
    for fanonima in t[4]:
        if fanonima != None:
            manipulaIdType(fanonima, listaIds, listaTipos, listaMetodos)

def manipulaExprLet(t, listaIds, listaTipos, listaMetodos):
    aux = ('featureDeclaration', t[1], t[2])
    manipulaFeatureDeclaration(aux, listaIds, listaTipos)
    for fanonima in t[3]:
        if fanonima != None:
...
def manipulafeatureAnonima(t, listaIds, listaTipos, listaMetodos):
    if isInListId(t[1], listaIds):
    ...
    if t[2] == 'Int':
        if type(t[3]) == tuple:
            if t[3][0] == 'op':
                chamaFuncao(t[3], listaIds, listaMetodos, listaTipos)
        elif type(t[3][1]) != int:
            raise SyntaxError("erro de declaração: valor incompativel com a variavel %s" % t[1])
    if t[2] == 'Bool':
        if type(t[3]) == tuple:
